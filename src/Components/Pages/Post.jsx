...
export default function Post() {
    ...
    const deletePost = () => {
        storageServiceServer.deletePost(post.$id).then((status) => {
            if (status) {
    ...
    const updatePost = () => {
        storageServiceServer.updatePost(slug, ...post).then((status) => {
            if (status) {
                storageServiceServer.uploadFile(post);
                    ...
                    {isAuthor && (
                        <div className="absolute right-6 top-6">
                            <Link to={`/edit-post/${post.$id}`}>
                                <Button bgColor="bg-green-500" className="mr-3" onClick={updatePost}>
                                    Edit
                                </Button>
                            </Link>
